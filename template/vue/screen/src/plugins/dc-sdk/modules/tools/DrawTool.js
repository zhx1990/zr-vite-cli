...
class DrawTool {
   ...
   */
  _onClick(e) {
    let position = this._options.clampToModel && e.position ? e.position : e.surfacePosition
    if (!position) {
      return false
    }
  ...
  _onCreateAnchor({ position, isCenter = false }) {
    return this._anchorLayer.entities.add({
      ...
      billboard: {
        ...
        heightReference:
          this._viewer.scene.mode === Cesium.SceneMode.SCENE3D && !this._options.clampToModel
