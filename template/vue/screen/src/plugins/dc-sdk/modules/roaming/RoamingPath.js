...
class RoamingPath {
  ...
  _onPostRender({ viewer, viewOption }) {
    ...
    if (Cesium.JulianDate.lessThan(now, this._endTime) && this._sampledPosition) {
  ...
  _resetTimeLine() {
    ...
    if (this._pathMode === 'speed') {
      ...
      timeLine = this._positions.map((item, index, arr) => {
        ...
        return Cesium.JulianDate.addSeconds(now, interval, new Cesium.JulianDate())
      })
    } else {
      let len = this._positions.length
      let interval = (this._duration - (this._duration % len)) / len
      timeLine = this._positions.map((item, index) => {
        return Cesium.JulianDate.addSeconds(now, index * interval, new Cesium.JulianDate())
      })
    }
