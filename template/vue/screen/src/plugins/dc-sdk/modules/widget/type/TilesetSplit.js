...
class TilesetSplit extends Widget {
  ...
  _unbindEvent() {
    if (this._tileset) {
      this._viewer.scene.splitPosition = this._tileset.splitDirection > 0 ? 1 : 0
    } else {
  ...
  _moveHandler(movement) {
    ...
    let splitPosition =
      (this._wrapper.offsetLeft + relativeOffset) / this._wrapper.parentElement.offsetWidth
  ...
  addTileset(tileset) {
    ...
    if (tileset) {
      ...
      this._tileset = this._viewer.scene.primitives.add(tileset.delegate || tileset)
