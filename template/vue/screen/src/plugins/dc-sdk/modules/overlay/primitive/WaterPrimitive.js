  ...
  set positions(positions) {
    ...
    this._delegate.geometryInstances.geometry = new Cesium.PolygonGeometry({
      polygonHierarchy: new Cesium.PolygonHierarchy(
        ...
        this._holes.map(
          (item) => new Cesium.PolygonHierarchy(Transform.transformWGS84ArrayToCartesianArray(item))
        )
      ),
  ...
  _setAppearance() {
    ...
    this._delegate.appearance = new Cesium.EllipsoidSurfaceAppearance({
      material: Cesium.Material.fromType('Water', {
        baseWaterColor: this._style?.baseWaterColor || new Cesium.Color(0.2, 0.3, 0.6, 1.0),
        blendColor: this._style?.blendColor || new Cesium.Color(0.0, 1.0, 0.699, 1.0),
