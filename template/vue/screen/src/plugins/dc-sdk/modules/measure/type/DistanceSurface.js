...
class DistanceSurface extends MeasureBase {
      ...
      this._getSampledHeight(lerpPositions)
            ...
            Cesium.Cartesian3.fromDegrees(
              ...
              Math.max(
                ...
                updatedCartesians[index]
                  ? Cesium.Cartographic.fromCartesian(updatedCartesians[index]).height
                  : 0
              )
            )
          )
        ...
        .then((positions) => {
          ...
          this._resultLabel.label.text =
            sum > 1000 ? `距离：${(sum / 1000).toFixed(2)} 公里` : `距离：${sum.toFixed(2)} 米`
        })
    }
  }
  ...
  start(measure, options) {
    ...
    this._startLabel.label.heightReference = Cesium.HeightReference.CLAMP_TO_GROUND
    this._resultLabel.label.heightReference = Cesium.HeightReference.CLAMP_TO_GROUND
    new DrawPolyline({
