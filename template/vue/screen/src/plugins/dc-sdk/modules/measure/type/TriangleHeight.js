...
class TriangleHeight extends MeasureBase {
  ...
  _onCalc(positions) {
    ...
    if (positions.length > 1) {
      let up = Cesium.Ellipsoid.WGS84.geodeticSurfaceNormal(positions[0], new Cesium.Cartesian3())
      ...
      this._lengthLabel.label.text =
        length > 1000 ? `${(length / 1000).toFixed(2)} 公里` : `${length.toFixed(2)} 米`
      ...
      this._distanceLabel.label.text =
        distance > 1000 ? `${(distance / 1000).toFixed(2)} 公里` : `${distance.toFixed(2)} 米`
      ...
      this._heightLabel.label.text =
        hegiht > 1000 ? `${(hegiht / 1000).toFixed(2)} 公里` : `${hegiht.toFixed(2)} 米`
    }
  }
