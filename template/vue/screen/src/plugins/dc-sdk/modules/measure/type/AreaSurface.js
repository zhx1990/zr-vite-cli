...
class AreaSurface extends MeasureBase {
  ...
  _lerp(rectangle, num = 4) {
    ...
    for (let y = 0; y < num; ++y) {
      for (let x = 0; x < num; ++x) {
        let longitude = Cesium.Math.lerp(rectangle.west, rectangle.east, x / (num - 1))
        let latitude = Cesium.Math.lerp(rectangle.south, rectangle.north, y / (num - 1))
  ...
  _onCalc(positions) {
    if (positions.length > 2) {
      this._resultLabel.position = Cesium.BoundingSphere.fromPoints(positions)?.center
      ...
      this._getSampledHeight(lerpPositions.concat(positions), false)
        ...
        .then((positions) => {
          this._resultLabel.label.text = `面积：${area(positions).toFixed(4)} 平方米`
        })
    }
  }
