...
class WindCanvas {
  ...
  _drawParticles() {
    ...
    if (this.field && len > 0) {
      ...
      for (; i < len; i++) {
        this[this.options.useCoordsDraw ? '_drawCoordsParticle' : '_drawPixelParticle'](
          particles[i],
          min,
          max
        )
  ...
  _drawStroke(pointPrev, pointNext, particle, min, max) {
    if (pointPrev && pointNext) {
      ...
      if (typeof this.options.colorScale === 'function') {
        ...
        let colorIdx = this._indexFor(particle.m, min, max, this.options.colorScale)
  ...
  setOptions(options) {
    ...
    if (!this.options?.paths && this.options?.particleMultiplier) {
      this.options.paths = Math.round(
        this.options.width * this.options.height * Number(this.options.particleMultiplier)
      )
    }
    return this
  }
