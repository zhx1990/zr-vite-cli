...
import { LayerGroupEventType, LayerEventType, MouseEvent, ViewerEvent, SceneEvent } from '../event'
...
class Viewer {
  ...
  get viewBounds() {
    let width = this.scene.canvas.width
    let height = this.scene.canvas.height
    let min = Transform.transformWindowToWGS84(new Cesium.Cartesian2(0, height), this)
    let max = Transform.transformWindowToWGS84(new Cesium.Cartesian2(width, 0), this)
    if (!min || !max) {
  ...
  setTerrain(terrain) {
    this._delegate.scene.setTerrain(
      new Cesium.Terrain(terrain || Promise.resolve(new Cesium.EllipsoidTerrainProvider()))
    )
    return this
  }
   ...
   * @return {Viewer}
   */
  flyToBounds(bounds, { heading = 0, pitch = 0, roll = 0 }, completeCallback, duration) {
    if (!bounds) {
      return this
    }
