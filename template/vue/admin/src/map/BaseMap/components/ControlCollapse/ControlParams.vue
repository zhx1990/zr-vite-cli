...
import { useMap, isReady } from '@/map'
...
const { getCurrentDistance, getCurrentLngLat, viewer } = useMap()
...
const onCameraChange = () => {
  ...
  params.distance = getCurrentDistance(geodesic)
}
const unwatch = watch(
  () => isReady.value,
  (val) => {
    if (val) {
      viewer.value?.on(DC.SceneEventType.CAMERA_CHANGED, onCameraChange)
      onCameraChange()
      setFalse()
      unwatch()
    }
...
onUnmounted(() => {
  viewer.value?.off(DC.SceneEventType.CAMERA_CHANGED, onCameraChange)
})
</script>
