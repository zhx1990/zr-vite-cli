...
class ClusterLayer extends Layer {
  ...
  _getClusterImage(count) {
    ...
    if (this._options.style === 'custom') {
      let keys = Object.keys(this._options.gradientImages).sort((a, b) => Number(a) - Number(b))
      ...
      let keys = Object.keys(this._options.gradientColors).sort((a, b) => Number(a) - Number(b))
      let color = undefined
      for (let i = keys.length - 1; i >= 0; i--) {
        if (rate >= Number(keys[i])) {
