...
class LightCylinderPrimitive extends Overlay {
  ...
  _updatePrimitives() {
    ...
    let topPositions = this._computeEllipsePositions(this._center, this._topRadius)
    let innerBottomPostions = this._computeEllipsePositions(this._center, this._bottomRadius * 0.7)
    let bottomPositions = this._computeEllipsePositions(this._center, this._bottomRadius)
    ...
    // ring
    let ring = isGroud ? new Cesium.GroundPrimitive(circleOpt) : new Cesium.Primitive(circleOpt)
    ...
    // circle
    let circle = isGroud ? new Cesium.GroundPrimitive(circleOpt) : new Cesium.Primitive(circleOpt)
    ...
    Cesium.Resource.fetchImage({ url: IMG_PARTICLES }).then((image) => {
      let particles = new Cesium.Primitive({
        ...
        appearance: new Cesium.EllipsoidSurfaceAppearance({
          material: Cesium.Material.fromType(Cesium.Material.CylinderParticlesType, {
            color: this._style.color,
