...
class DynamicOverlay extends Overlay {
  ...
  _removePosition() {
    if (this._cache.length > this._maxCacheSize) {
      let start = Cesium.JulianDate.addSeconds(this._cache[0], -0.2, new Cesium.JulianDate())
  ...
  addPosition(position, interval) {
    ...
    let time = Cesium.JulianDate.addSeconds(now, interval, new Cesium.JulianDate())
