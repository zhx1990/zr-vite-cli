...
class ContextMenu extends Widget {
  ...
  _onRightClick(movement) {
    ...
    if (scene.mode === Cesium.SceneMode.SCENE3D) {
      ...
      this._surfacePosition = scene.camera.pickEllipsoid(movement.position, Cesium.Ellipsoid.WGS84)
    }

    if (this._surfacePosition) {
      let c = Cesium.Ellipsoid.WGS84.cartesianToCartographic(this._surfacePosition)
      if (c) {
    ...
    if (target?.id instanceof Cesium.Entity) {
      let layer = this._viewer.getLayers().filter((item) => item.layerId === target.id.layerId)[0]
   ...
   */
  _setCustomClass() {
    DomUtil.setClass(this._wrapper, `dc-context-menu ${this._config.customClass}`)
  }
