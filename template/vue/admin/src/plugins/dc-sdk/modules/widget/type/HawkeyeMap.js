...
class HawkeyeMap extends Widget {
  ...
  _syncMap() {
    ...
    let distance = Cesium.Cartesian3.distance(worldPosition, this._viewer.scene.camera.positionWC)
    this._map.scene.camera.lookAt(worldPosition, new Cesium.Cartesian3(0.0, 0.0, distance))
  }
  ...
  addBaseLayer(baseLayer) {
    ...
    if (baseLayer) {
      ...
      baseLayer.forEach((item) => {
        this._map.imageryLayers.add(Cesium.ImageryLayer.fromProviderAsync(item, {}))
      })
    }
    return this
  }
