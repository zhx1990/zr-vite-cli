...
class Compass extends Widget {
  ...
  _mountContent() {
    ...
    this._rotation_marker = DomUtil.parseDom(Icons.compass_rotation_marker, true, 'rotation_marker')
  ...
  _rotateMouseMoveFunction(e) {
    ...
    let newCameraAngle = Cesium.Math.zeroToTwoPi(this._rotateInitialCameraAngle - angleDifference)
    let oldTransform
    if (this._rotateFrame) {
      oldTransform = Cesium.Matrix4.clone(camera.transform)
      camera.lookAtTransform(this._rotateFrame)
