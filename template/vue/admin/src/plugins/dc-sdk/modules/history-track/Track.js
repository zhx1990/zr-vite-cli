...
class Track {
  constructor(positions, duration, callback, options) {
    ...
    this._trackEvent.on(TrackEventType.RESET_TIME_LINE, this._resetTimeLine, this)
    ...
    if (Cesium.JulianDate.lessThanOrEquals(now, this._endTime)) {
      ...
      if (time) {
        ...
        if (timeDiff >= 0 && timeDiff <= 1) {
          ...
          this._callback &&
            this._callback(position, this._positionIndex + 1 === this._positions.length)
          this._positionIndex++
        }
      }
    }
  ...
  _resetTimeLine(params) {
    ...
    if (!params?.stopTime && !params?.duration) {
      ...
      this._timeLine = this._positions.map((item, index, arr) => {
        ...
        return Cesium.JulianDate.addSeconds(this._startTime, interval, new Cesium.JulianDate())
      })
      this._pathPositions = []
      this._positionIndex = 0
      ...
      this._timeLine = this._timeLine.map((item) => {
        if (Cesium.JulianDate.greaterThan(item, params.stopTime)) {
          item = Cesium.JulianDate.addSeconds(item, params.duration, new Cesium.JulianDate())
        }
        return item
      })
    }
    ...
    this._sampledPosition.forwardExtrapolationType = Cesium.ExtrapolationType.HOLD
    ...
    this._velocityOrientation = new Cesium.VelocityOrientationProperty(this._sampledPosition)
