...
class MouseEvent extends Event {
  ...
  _raiseEvent(type, mouseInfo = {}) {
    ...
    // get Viewer Event
    if ((!event || event.numberOfListeners === 0) && this._viewer?.viewerEvent) {
    ...
    if (overlay?.allowDrillPicking) {
      ...
      drillInfos.forEach((drillInfo) => {
        ...
        if (dillOverlay?.overlayId !== overlay.overlayId && dillOverlay?.overlayEvent) {
          ...
          // get Layer Event
          if ((!event || event.numberOfListeners === 0) && dillLayer?.layerEvent) {
  ...
  _clickHandler(movement) {
    ...
    this._raiseEvent(MouseEventType.CLICK, this._getMouseInfo(movement.position))
  }
  ...
  _dbClickHandler(movement) {
    ...
    this._raiseEvent(MouseEventType.DB_CLICK, this._getMouseInfo(movement.position))
  }
  ...
  _rightClickHandler(movement) {
    ...
    this._raiseEvent(MouseEventType.RIGHT_CLICK, this._getMouseInfo(movement.position))
  }
  ...
  _mouseMoveHandler(movement) {
    ...
    if (
      !this._selected ||
      this._getOverlayId(this._selected.target) !== this._getOverlayId(mouseInfo.target)
    ) {
  ...
  _leftDownHandler(movement) {
    ...
    this._raiseEvent(MouseEventType.LEFT_DOWN, this._getMouseInfo(movement.position))
  }
  ...
  _rightDownHandler(movement) {
    ...
    this._raiseEvent(MouseEventType.RIGHT_DOWN, this._getMouseInfo(movement.position))
  }
