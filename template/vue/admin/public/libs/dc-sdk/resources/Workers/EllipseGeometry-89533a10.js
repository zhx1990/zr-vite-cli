define([
  ...
  './EllipseGeometryLibrary-b0d7df17',
    ...
    if (vertexFormat.st) {
      ...
      for (let k = 0; k < length; k += 2) {
        textureCoordinates[k] =
          (textureCoordinates[k] - minTexCoord.x) / (maxTexCoord.x - minTexCoord.x)
        textureCoordinates[k + 1] =
          (textureCoordinates[k + 1] - minTexCoord.y) / (maxTexCoord.y - minTexCoord.y)
      }
    }
  ...
  function computeWallAttributes(positions, options) {
    ...
    for (let i = 0; i < length; i += 3) {
      ...
      let scaledNormal = Matrix3.Cartesian3.multiplyByScalar(normal, height, scratchCartesian4)
      position = Matrix3.Cartesian3.add(position, scaledNormal, position)
      scaledNormal = Matrix3.Cartesian3.multiplyByScalar(normal, extrudedHeight, scaledNormal)
      extrudedPosition = Matrix3.Cartesian3.add(extrudedPosition, scaledNormal, extrudedPosition)
      ...
      if (vertexFormat.normal || vertexFormat.tangent || vertexFormat.bitangent) {
        ...
        const next = Matrix3.Cartesian3.fromArray(positions, (i + 3) % length, scratchCartesian4)
        Matrix3.Cartesian3.subtract(next, position, next)
        const bottom = Matrix3.Cartesian3.subtract(extrudedPosition, position, scratchCartesian3)
  ...
  function computeExtrudedEllipse(options) {
    ...
    const boundingSphere = Transforms.BoundingSphere.union(topBoundingSphere, bottomBoundingSphere)
