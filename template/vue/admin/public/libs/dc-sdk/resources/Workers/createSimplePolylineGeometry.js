define([
  ...
  SimplePolylineGeometry.createGeometry = function (simplePolylineGeometry) {
    ...
    const minDistance = Math.CesiumMath.chordLength(granularity, ellipsoid.maximumRadius)
    ...
    if (arcType === ArcType.ArcType.GEODESIC || arcType === ArcType.ArcType.RHUMB) {
      ...
      if (arcType === ArcType.ArcType.GEODESIC) {
        subdivisionSize = Math.CesiumMath.chordLength(granularity, ellipsoid.maximumRadius)
      ...
      if (perSegmentColors) {
        ...
        for (i = 0; i < length - 1; i++) {
          positionCount +=
            numberOfPointsFunction(positions[i], positions[i + 1], subdivisionSize) + 1
        }
      ...
      colorValues = defaultValue.defined(colors) ? new Uint8Array(numberOfPositions * 4) : undefined
    ...
    const indices = IndexDatatype.IndexDatatype.createTypedArray(numberOfPositions, numberOfIndices)
  ...
  function createSimplePolylineGeometry(simplePolylineGeometry, offset) {
    ...
    simplePolylineGeometry._ellipsoid = Matrix3.Ellipsoid.clone(simplePolylineGeometry._ellipsoid)
