define([
  ...
  function decodePositions(
    ...
    for (let i = 0; i < positionsLength; ++i) {
      ...
      const lat = Math$1.CesiumMath.lerp(rectangle.south, rectangle.north, v / MAX_SHORT)
  ...
  function computeMiteredNormal(
    ...
    const towardNext = Matrix3.Cartesian3.subtract(nextPosition, position, towardNextScratch)
    let towardCurr = Matrix3.Cartesian3.subtract(position, previousPosition, towardCurrScratch)
    ...
    if (Matrix3.Cartesian3.dot(towardNext, towardCurr) < MITER_BREAK) {
      towardCurr = Matrix3.Cartesian3.multiplyByScalar(towardCurr, -1.0, towardCurrScratch)
    }
  ...
  VertexAttributesAndIndices.prototype.addVolume = function (
    ...
    const endEllipsoidNormal = ellipsoid.geodeticSurfaceNormal(position, scratchEndEllipsoidNormal)
    ...
    for (i = 0; i < 8; i++) {
      ...
      Matrix3.Cartesian3.pack(startFaceNormal, startFaceNormalAndVertexCornerIds, vec4Offset)
  ...
  function createVectorTileClampedPolylines(parameters, transferableObjects) {
    ...
    const vBuffer = encodedPositions.subarray(positionsLength, 2 * positionsLength)
    const heightBuffer = encodedPositions.subarray(2 * positionsLength, 3 * positionsLength)
    ...
    for (i = 0; i < countsLength; i++) {
      ...
      for (let j = 0; j < polylineVolumeCount; j++) {
        const volumeStart = Matrix3.Cartesian3.unpack(positionsRTC, currentPositionIndex, scratchP0)
        ...
        startHeight = Math$1.CesiumMath.lerp(minimumHeight, maximumHeight, startHeight / MAX_SHORT)
        endHeight = Math$1.CesiumMath.lerp(minimumHeight, maximumHeight, endHeight / MAX_SHORT)
        ...
        if (j === 0) {
          ...
          if (Matrix3.Cartesian3.equals(finalPosition, volumeStart)) {
            ...
            const offsetPastStart = Matrix3.Cartesian3.subtract(volumeStart, volumeEnd, scratchPrev)
        ...
        if (j === polylineVolumeCount - 1) {
          ...
          if (Matrix3.Cartesian3.equals(firstPosition, volumeEnd)) {
            Matrix3.Cartesian3.unpack(positionsRTC, volumeFirstPositionIndex + 3, postEnd)
          } else {
            const offsetPastEnd = Matrix3.Cartesian3.subtract(volumeEnd, volumeStart, scratchNext)
