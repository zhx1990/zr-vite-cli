define([
  ...
  EllipsoidalOccluder.prototype.computeHorizonCullingPointFromRectangle = function (
    ...
    const positions = Matrix2.Rectangle.subsample(rectangle, ellipsoid, 0.0, subsampleScratch)
  ...
  function isScaledSpacePointVisible(
    ...
    const vt = Matrix3.Cartesian3.subtract(occludeeScaledSpacePosition, cv, scratchCartesian)
    ...
    const isOccluded =
      vhMagnitudeSquared < 0
        ...
        : vtDotVc > vhMagnitudeSquared &&
          (vtDotVc * vtDotVc) / Matrix3.Cartesian3.magnitudeSquared(vt) > vhMagnitudeSquared
    return !isOccluded
  }
  ...
  function magnitudeToPoint(scaledSpaceDirectionToPoint, resultMagnitude, result) {
    ...
    return Matrix3.Cartesian3.multiplyByScalar(scaledSpaceDirectionToPoint, resultMagnitude, result)
  }
  ...
  function TerrainEncoding(
    ...
    if (
      ...
      const dimensions = Matrix3.Cartesian3.subtract(maximum, minimum, cartesian3DimScratch)
     ...
     * @type {boolean}
     */
    this.hasGeodeticSurfaceNormals = defaultValue.defaultValue(hasGeodeticSurfaceNormals, false)
     ...
     */
    this.exaggerationRelativeHeight = defaultValue.defaultValue(exaggerationRelativeHeight, 0.0)
  ...
  TerrainEncoding.prototype.encode = function (
    ...
    if (this.quantization === TerrainQuantization$1.BITS12) {
      position = Matrix2.Matrix4.multiplyByPoint(this.toScaledENU, position, cartesian3Scratch)
      ...
      const compressed0 =
        AttributeCompression.AttributeCompression.compressTextureCoordinates(cartesian2Scratch)
  ...
  TerrainEncoding.prototype.getExaggeratedPosition = function (buffer, index, result) {
    ...
    if (hasExaggeration && this.hasGeodeticSurfaceNormals) {
      ...
      const heightDifference =
        TerrainExaggeration$1.getHeight(rawHeight, exaggeration, exaggerationRelativeHeight) -
        rawHeight
