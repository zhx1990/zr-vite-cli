define([
  ...
  PolylinePipeline.numberOfPointsRhumbLine = function (p0, p1, granularity) {
    const radiansDistanceSquared =
      Math.pow(p0.longitude - p1.longitude, 2) + Math.pow(p0.latitude - p1.latitude, 2)
  ...
  PolylinePipeline.wrapLongitude = function (positions, modelMatrix) {
    ...
    if (defaultValue.defined(positions) && positions.length > 0) {
      ...
      const xzPlane = Plane.Plane.fromPointNormal(origin, xzNormal, wrapLongitudeXZPlane)
      ...
      const yzPlane = Plane.Plane.fromPointNormal(origin, yzNormal, wrapLongitudeYZPlane)
      ...
      for (let i = 1; i < length; ++i) {
        ...
        if (
          ...
          if (defaultValue.defined(intersection)) {
            ...
            cartesians.push(Matrix3.Cartesian3.add(intersection, offset, new Matrix3.Cartesian3()))
