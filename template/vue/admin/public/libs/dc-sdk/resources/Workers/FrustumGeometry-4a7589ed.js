define([
  ...
  FrustumGeometry.unpack = function (array, startingIndex, result) {
    ...
    startingIndex += Matrix3.Cartesian3.packedLength
    const orientation = Transforms.Quaternion.unpack(array, startingIndex, scratchPackQuaternion)
    startingIndex += Transforms.Quaternion.packedLength
  ...
  FrustumGeometry._computeNearFarPlanes = function (
    ...
    const rotationMatrix = Matrix3.Matrix3.fromQuaternion(orientation, scratchRotationMatrix)
    ...
    if (frustumType === PERSPECTIVE) {
      const viewProjection = Matrix2.Matrix4.multiply(projection, view, scratchInverseMatrix)
      inverseViewProjection = Matrix2.Matrix4.inverse(viewProjection, scratchInverseMatrix)
    } else {
    ...
    for (let i = 0; i < 2; ++i) {
      for (let j = 0; j < 4; ++j) {
        let corner = Matrix2.Cartesian4.clone(frustumCornersNDC[j], scratchFrustumCorners[j])
        ...
        if (!defaultValue.defined(inverseViewProjection)) {
          ...
          corner.x =
            (corner.x * (frustum.right - frustum.left) + frustum.left + frustum.right) * 0.5
          corner.y =
            (corner.y * (frustum.top - frustum.bottom) + frustum.bottom + frustum.top) * 0.5
          corner.z = (corner.z * (near - far) - near - far) * 0.5
          corner.w = 1.0

          Matrix2.Matrix4.multiplyByVector(inverseView, corner, corner)
        } else {
          corner = Matrix2.Matrix4.multiplyByVector(inverseViewProjection, corner, corner)
