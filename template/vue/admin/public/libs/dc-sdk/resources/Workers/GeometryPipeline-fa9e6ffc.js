define([
  ...
  GeometryPipeline.projectTo2D = function (
    ...
    for (let i = 0; i < values3D.length; i += 3) {
      const value = Matrix3.Cartesian3.fromArray(values3D, i, scratchProjectTo2DCartesian3)
  ...
  function transformVector(matrix, attribute) {
    if (defaultValue.defined(attribute)) {
      ...
      for (let i = 0; i < length; i += 3) {
        ...
        scratchCartesian3 = Matrix3.Cartesian3.normalize(scratchCartesian3, scratchCartesian3)
        Matrix3.Cartesian3.pack(scratchCartesian3, values, i)
      }
    }
  }
  ...
  function indexTriangles(geometry) {
    ...
    const indices = IndexDatatype.IndexDatatype.createTypedArray(numberOfVertices, numberOfVertices)
  ...
  const interpolateAndPackCartesian4 = generateBarycentricInterpolateFunction(Matrix2.Cartesian4, 4)
  const interpolateAndPackCartesian3 = generateBarycentricInterpolateFunction(Matrix3.Cartesian3, 3)
  const interpolateAndPackCartesian2 = generateBarycentricInterpolateFunction(Matrix2.Cartesian2, 2)
    ...
    switch (componentsPerAttribute) {
      ...
      default:
        currentValues[insertedIndex] =
          sourceValues[i0] * coords.x + sourceValues[i1] * coords.y + sourceValues[i2] * coords.z
    }
  }
  ...
  function splitLongitudeTriangles(instance) {
    ...
    const normals = defaultValue.defined(attributes.normal) ? attributes.normal.values : undefined
  ...
  function splitLongitudeLines(instance) {
    ...
    for (i = 0; i < length; i += 2) {
      ...
      if (defaultValue.defined(intersection)) {
        ...
        const offsetPoint = Matrix3.Cartesian3.add(intersection, offset, offsetPointScratch)
  ...
  function updateAdjacencyAfterSplit(geometry) {
    ...
    for (let j = 0; j < length; j += 3) {
      ...
      const prevPosition = Matrix3.Cartesian3.unpack(prevPositions, j, cartesian3Scratch2)
      if (
      ...
      const nextPosition = Matrix3.Cartesian3.unpack(nextPositions, j, cartesian3Scratch3)
      if (
  ...
  function splitLongitudePolyline(instance) {
    ...
    const texCoords = defaultValue.defined(attributes.st) ? attributes.st.values : undefined
    const colors = defaultValue.defined(attributes.color) ? attributes.color.values : undefined
    ...
    for (i = 0; i < length; i += 4) {
      ...
      if (defaultValue.defined(intersection)) {
        ...
        const offsetPoint = Matrix3.Cartesian3.add(intersection, offset, cartesian3Scratch6)
        ...
        const ew0 = Matrix2.Cartesian2.fromArray(expandAndWidths, i0 * 2, cartesian2Scratch0)
        ...
        if (defaultValue.defined(texCoords)) {
          ...
          const s3 = Matrix2.Cartesian2.fromArray(texCoords, (i + 3) * 2, cartesian2Scratch1)
