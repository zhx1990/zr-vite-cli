define([
  ...
  './Math-0a2ac845',
  './Matrix2-e1298525',
], function (exports, Matrix3, defaultValue, Check, Transforms, Math$1, Matrix2) {
  'use strict'
  ...
  RectangleGeometryLibrary.computePosition = function (
    ...
    let stLatitude =
      nwCorner.latitude - computedOptions.granYCos * row + col * computedOptions.granXSin
    const cosLatitude = cos(stLatitude)
    const nZ = sin(stLatitude)
    const kZ = radiiSquared.z * nZ

    let stLongitude =
      nwCorner.longitude + row * computedOptions.granYSin + col * computedOptions.granXCos
    ...
    if (computeST) {
      ...
      if (defaultValue.defined(stNwCorner)) {
        stLatitude =
          stNwCorner.latitude - computedOptions.stGranYCos * row + col * computedOptions.stGranXSin
        stLongitude =
          stNwCorner.longitude + row * computedOptions.stGranYSin + col * computedOptions.stGranXCos
  ...
  RectangleGeometryLibrary.computeOptions = function (
    ...
    const boundingRectangle = Matrix2.Rectangle.clone(rectangle, boundingRectangleScratch)
