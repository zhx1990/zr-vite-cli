define([
  ...
  PolygonGeometryLibrary.polygonsFromHierarchy = function (
    ...
    while (queue.length !== 0) {
      ...
      let originalWindingOrder = PolygonPipeline.PolygonPipeline.computeWindingOrder2D(positions2D)
      ...
      for (i = 0; i < numChildren; i++) {
        ...
        originalWindingOrder =
          PolygonPipeline.PolygonPipeline.computeWindingOrder2D(holePositions2D)
        if (originalWindingOrder === PolygonPipeline.WindingOrder.CLOCKWISE) {
  ...
  PolygonGeometryLibrary.computeBoundingRectangle = function (
    ...
    const textureMatrix = Matrix3.Matrix3.fromQuaternion(rotation, computeBoundingRectangleMatrix3)
    ...
    for (let i = 0; i < length; ++i) {
      const p = Matrix3.Cartesian3.clone(positions[i], computeBoundingRectangleCartesian3)
