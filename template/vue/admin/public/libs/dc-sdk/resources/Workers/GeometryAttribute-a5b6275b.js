define([
  ...
  './Transforms-b527bb09',
], function (exports, Matrix2, Matrix3, Check, defaultValue, WebGLConstants, Transforms) {
  'use strict'
  ...
  Geometry._textureCoordinateRotationPoints = function (
    ...
    const rectangleCenter = Matrix2.Rectangle.center(boundingRectangle, rectangleCenterScratch)
    const enuCenter = Matrix3.Cartographic.toCartesian(rectangleCenter, ellipsoid, enuCenterScratch)
    const enuToFixedFrame = Transforms.Transforms.eastNorthUpToFixedFrame(
      enuCenter,
      ellipsoid,
      fixedFrameToEnuScratch
    )
    const fixedFrameToEnu = Matrix2.Matrix4.inverse(enuToFixedFrame, fixedFrameToEnuScratch)
    ...
    const textureMatrix = Matrix3.Matrix3.fromQuaternion(rotation, enuRotationMatrixScratch)
    ...
    for (i = 0; i < positionsLength; i++) {
      posEnu = Matrix2.Matrix4.multiplyByPointAsVector(fixedFrameToEnu, positions[i], posEnu)
    ...
    const toDesiredInComputed = Matrix2.Matrix2.fromRotation(stRotation, rotation2DScratch)
