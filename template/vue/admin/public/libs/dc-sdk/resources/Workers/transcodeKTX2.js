define([
...
], function (defaultValue, Check, WebGLConstants, RuntimeError, createTaskProcessorWorker) {
  'use strict'
  ...
  */
  ...
  function read(data) {
    ...
    const levelReader = new BufferReader(
      data,
      KTX2_ID.length + headerByteLength,
      levelByteLength,
      true
    )
    ...
    while (kvdReader._offset < kvdByteLength) {
      ...
      container.keyValue[key] = kvdReader._nextUint8Array(
        keyValueByteLength - keyData.byteLength - 1
      )
    ...
    const endpointsData = new Uint8Array(
      data.buffer,
      data.byteOffset + endpointsByteOffset,
      endpointsByteLength
    )
    const selectorsData = new Uint8Array(
      data.buffer,
      data.byteOffset + selectorsByteOffset,
      selectorsByteLength
    )
    const tablesData = new Uint8Array(
      data.buffer,
      data.byteOffset + tablesByteOffset,
      tablesByteLength
    )
    const extendedData = new Uint8Array(
      data.buffer,
      data.byteOffset + extendedByteOffset,
      extendedByteLength
    )
  ...
  const faceOrder = ['positiveX', 'negativeX', 'positiveY', 'negativeY', 'positiveZ', 'negativeZ']
  ...
  function parseUncompressed(header, result) {
    ...
    for (let i = 0; i < header.levels.length; ++i) {
      ...
      const faceLength = width * height * PixelFormat$1.componentsLength(internalFormat)
