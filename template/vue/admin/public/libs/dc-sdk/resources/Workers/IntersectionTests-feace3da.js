define([
  ...
  './Transforms-b527bb09',
  './Math-0a2ac845',
], function (exports, Matrix3, defaultValue, Check, Transforms, Math$1) {
  'use strict'
  ...
  function original(a3, a2, a1, a0) {
    ...
    const cubicRoots = CubicRealPolynomial$1.computeRealRoots(1.0, 2.0 * p, p * p - 4.0 * r, -q * q)
  ...
  function Ray(origin, direction) {
    direction = Matrix3.Cartesian3.clone(
      defaultValue.defaultValue(direction, Matrix3.Cartesian3.ZERO)
    )
     ...
     */
    this.origin = Matrix3.Cartesian3.clone(
      defaultValue.defaultValue(origin, Matrix3.Cartesian3.ZERO)
    )
  ...
  IntersectionTests.rayEllipsoid = function (ray, ellipsoid) {
    ...
    const w = Matrix3.Cartesian3.multiplyComponents(inverseRadii, ray.direction, scratchW)
  ...
  function quadraticVectorExpression(A, b, c, x, w) {
    ...
    for (let i = 0; i < length; ++i) {
      ...
      const right = addWithCancellationCheck(r1 * cosine, r0, Math$1.CesiumMath.EPSILON15)
  ...
  IntersectionTests.grazingAltitudeLocation = function (ray, ellipsoid) {
    ...
    const A = Matrix3.Matrix3.multiply(Matrix3.Matrix3.multiply(temp, D_I, aScratch), B, aScratch)
  ...
  IntersectionTests.lineSegmentPlane = function (endPoint0, endPoint1, plane, result) {
    ...
    const difference = Matrix3.Cartesian3.subtract(endPoint1, endPoint0, lineSegmentPlaneDifference)
    const normal = plane.normal
